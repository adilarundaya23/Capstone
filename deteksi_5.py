...
cred = credentials.Certificate("serviceAccountKey.json")  # Pastikan file ini ada
...
dbase = client["SPK"]  # Nama database
collection_face = dbase["presensi"]  # Collection untuk data face recognition
collection_activity = dbase["tracking"]  # Collection untuk data aktivitas
...
def run(
    ...
    meja1_x1, meja1_y1 = 125, 327
    meja1_x2, meja1_y2 = 125, 327
    meja2_x1, meja2_y1 = 320, 349
    meja2_x2, meja2_y2 = 320, 349
    meja3_x1, meja3_y1 = 500, 359
    meja3_x2, meja3_y2 = 500, 359
    meja4_x1, meja4_y1 = 236, 209
    meja4_x2, meja4_y2 = 236, 209
    meja5_x1, meja5_y1 = 348, 199
    meja5_x2, meja5_y2 = 348, 199
    meja6_x1, meja6_y1 = 433, 210
    meja6_x2, meja6_y2 = 433, 210
    prev_time = time.time()
    ...
    sleep_count_meja5 = 0
    sleep_count_meja6 = 0
    sleep_in_table1 = False
    sleep_in_table2 = False
    sleep_in_table3 = False
    sleep_in_table4 = False
    sleep_in_table5 = False
    sleep_in_table6 = False
    ...
    # Fungsi mengambil token sesuai meja dari Firestore
    def get_meja_token(nama_meja):
        doc_ref = db.collection("data_meja").document("meja_tokens")
        doc = doc_ref.get()
        if doc.exists:
            data = doc.to_dict()
            return data.get(nama_meja, None)
        return None

    # Fungsi untuk mengirim notifikasi FCM
    def send_fcm_notification_for_meja(nama_meja):
        token = get_meja_token(nama_meja)
        if token:
            message = messaging.Message(
                notification=messaging.Notification(
                    title="Deteksi Tidur 😴",
                    body=f"Ada mahasiswa tertidur di {nama_meja}",
                ),
                token=token,
            )
            response = messaging.send(message)
            print(f"Notifikasi terkirim ke {nama_meja}: {response}")
        else:
            print(f"Token untuk {nama_meja} tidak ditemukan.")

    for path, im, im0s, vid_cap, s in dataset:
        start_time = time.time()
        current_time = time.time()
        is_in_table1 = is_in_table2 = is_in_table3 = is_in_table4 = is_in_table5 = (
            is_in_table6
        ) = False
        ...
        def save_results_to_json():
            results = {
                ...
                "total_time_meja4": counts_meja4,
                "total_time_meja5": counts_meja5,
                "total_time_meja6": counts_meja6,
            }
        ...
        # Inisialisasi timer tidur untuk setiap meja
        sleep_timers = {
            "Meja 1": 0,
            "Meja 2": 0,
            "Meja 3": 0,
        ...
        sleep_threshold = 5  # detik
        notified_tables = set()  # Untuk mencegah spam
        ...
        for i, det in enumerate(pred):
            ...
            if len(det):
                ...
                table5_points = generate_table_points(
                    meja5_x1, meja5_y1, meja5_x2, meja5_y2
                )
                table6_points = generate_table_points(
                    meja6_x1, meja6_y1, meja6_x2, meja6_y2
                )
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if label == "sit":
                        ...
                        for px, py in table5_points:
                            if is_point_in_box(
                                px, py, xyxy[0], xyxy[1], xyxy[2], xyxy[3]
                            ):
                                is_in_table5 = True

                        for px, py in table6_points:
                            if is_point_in_box(
                                px, py, xyxy[0], xyxy[1], xyxy[2], xyxy[3]
                            ):
                                is_in_table6 = True
                        ...
                        for px, py in table1_points:
                            if is_point_in_box(
                                ...
                                sleep_in_table1 = True
                        for px, py in table2_points:
                            if is_point_in_box(
                                px, py, xyxy[0], xyxy[1], xyxy[2], xyxy[3]
                            ):
                                sleep_in_table2 = True
                        for px, py in table3_points:
                            if is_point_in_box(
                                px, py, xyxy[0], xyxy[1], xyxy[2], xyxy[3]
                            ):
                                sleep_in_table3 = True
                        for px, py in table4_points:
                            if is_point_in_box(
                                px, py, xyxy[0], xyxy[1], xyxy[2], xyxy[3]
                            ):
                                sleep_in_table4 = True
                        for px, py in table5_points:
                            if is_point_in_box(
                                px, py, xyxy[0], xyxy[1], xyxy[2], xyxy[3]
                            ):
                                sleep_in_table5 = True
                        for px, py in table6_points:
                            if is_point_in_box(
                                px, py, xyxy[0], xyxy[1], xyxy[2], xyxy[3]
                            ):
                                sleep_in_table6 = True
                ...
                meja5_text = "Yes" if is_in_table5 else "No"
                meja6_text = "Yes" if is_in_table6 else "No"

            # Perbarui counts setiap 1 detik
            if current_time - last_count_time >= 1:
                if is_in_table1 and not sleep_in_table1:
                    counts_meja1 += 1
                elif sleep_in_table1:
                    sleep_count_meja1 += 1
                    sleep_meja1 += 1
                    if sleep_meja1 == 5:
                        send_fcm_notification_for_meja("Meja 1")
                        sleep_meja1 = 0

                if is_in_table2 and not sleep_in_table2:
                    counts_meja2 += 1
                elif sleep_in_table2:
                    sleep_count_meja2 += 1
                    sleep_meja2 += 1
                    if sleep_meja2 == 5:
                        send_fcm_notification_for_meja("Meja 2")
                        sleep_meja2 = 0

                if is_in_table3 and not sleep_in_table3:
                    counts_meja3 += 1
                elif sleep_in_table3:
                    sleep_count_meja3 += 1
                    sleep_meja3 += 1
                    if sleep_meja3 == 5:
                        send_fcm_notification_for_meja("Meja 3")
                        sleep_meja3 = 0

                if is_in_table4 and not sleep_in_table4:
                    counts_meja4 += 1
                elif sleep_in_table4:
                    sleep_count_meja4 += 1
                    sleep_meja4 += 1
                    if sleep_meja4 == 5:
                        send_fcm_notification_for_meja("Meja 4")
                        sleep_meja4 = 0

                if is_in_table5 and not sleep_in_table5:
                    counts_meja5 += 1
                elif sleep_in_table5:
                    sleep_count_meja5 += 1
                    sleep_meja5 += 1
                    if sleep_meja5 == 5:
                        send_fcm_notification_for_meja("Meja 5")
                        sleep_meja5 = 0

                if is_in_table6 and not sleep_in_table6:
                    counts_meja6 += 1
                elif sleep_in_table6:
                    sleep_count_meja6 += 1
                    sleep_meja6 += 1
                    if sleep_meja6 == 5:
                        send_fcm_notification_for_meja("Meja 6")
                        sleep_meja6 = 0
                ...
                if current_time - last_save_time >= 10:
                    ...
                    results = {
                        "meja1": counts_meja1,
                        "sleepmeja1": sleep_count_meja1,
                        "meja2": counts_meja2,
                        "sleepmeja2": sleep_count_meja2,
                        "meja3": counts_meja3,
                        "sleepmeja3": sleep_count_meja3,
                        "meja4": counts_meja4,
                        "sleepmeja4": sleep_count_meja4,
                        "meja5": counts_meja5,
                        "sleepmeja5": sleep_count_meja5,
                        "meja6": counts_meja6,
                        "sleepmeja6": sleep_count_meja6,
                        "time": round(current_time - timer, 1),
                    }
                    # Menyimpan data aktivitas ke MongoDB
                    # collection_activity.delete_many({})
            ...
            cv2.rectangle(
                im0, (meja5_x1, meja5_y1), (meja5_x2, meja5_y2), (0, 255, 0), 2
            )
            cv2.rectangle(
                im0, (meja6_x1, meja6_y1), (meja6_x2, meja6_y2), (0, 255, 0), 2
            )
            cv2.putText(
                im0,
            ...
            cv2.putText(
                im0,
                f"{meja5_text}: {counts_meja5}",
                (meja5_x1, meja5_y1 - 10),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.5,
                (255, 0, 0),
                2,
            ...
            cv2.putText(
                im0,
                f"{meja6_text}: {counts_meja6}",
                (meja6_x1, meja6_y1 - 10),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.5,
                (255, 0, 0),
                2,
    ...
    print(f"Waktu total di Meja 5: {counts_meja5} detik")
    print(f"Waktu total di Meja 6: {counts_meja6} detik")
